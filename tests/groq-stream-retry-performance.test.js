...
const source = fs.readFileSync(
  path.join(__dirname, "..", "Addon_AI_Groq.js"),
  "utf8"
);
...
assert.ok(baseUrlMatch, "missing Groq BASE_URL");
...
const parentFunctionSource = productionFunctionSource
  .replace(
    "async function callGroqAPIStream(",
    "async function callGroqAPIStreamReference("
  )
  .replace(
...
function createHarness(functionSource, functionName, includeEmitter) {
  ...
  vm.runInContext(
    ...
    context,
    { filename: `Addon_AI_Groq.${functionName}.extracted.js` }
  );
  return {
...
function createRuntime(scenario) {
  ...
  return {
    ...
    getSelectedModel() {
      ...
      return scenario.model || "llama-test";
    },
...
test("Groq addon remains a Spotify-wide extension", () => {
  assert.equal(manifest.subfiles_extension.includes("Addon_AI_Groq.js"), true);
  assert.equal(manifest.subfiles.includes("Addon_AI_Groq.js"), false);
});

test("full Groq SSE flow preserves HTTP, key, retry, and callback semantics", async () => {
...
test("full Groq flow preserves callback exception retry order", async () => {
  const harnesses = createHarnesses();
...
test("full Groq SSE byte-split Unicode fuzz matches the parent", async () => {
  const harnesses = createHarnesses();
